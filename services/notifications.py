...
def create_notification(db: Database, notification_data: dict) -> dict:
    """Create a new notification.

    Args:
        db (Database): MongoDB database instance.
        notification_data (dict): Data for the notification including type, message, user_id, and vendor_id.

    Returns:
        dict: Dictionary containing the created notification ID.
    ...
    """
    try:
        if not notification_data.get("type") or not notification_data.get("message"):
            raise ValidationError("Type and message are required")
        if not notification_data.get("user_id") and not notification_data.get("vendor_id"):
            raise ValidationError("Either user_id or vendor_id must be provided")

        if notification_data.get("user_id"):
            if not ObjectId.is_valid(notification_data["user_id"]):
                raise ValidationError(f"Invalid user_id format: {notification_data['user_id']}")
            if not db.users.find_one({"_id": ObjectId(notification_data["user_id"])}):
                raise NotFoundError(f"User with ID {notification_data['user_id']} not found")
        if notification_data.get("vendor_id"):
            if not ObjectId.is_valid(notification_data["vendor_id"]):
                raise ValidationError(f"Invalid vendor_id format: {notification_data['vendor_id']}")
            if not db.vendors.find_one({"_id": ObjectId(notification_data["vendor_id"])}):
                raise NotFoundError(f"Vendor with ID {notification_data['vendor_id']} not found")
        ...
        logger.error("Duplicate notification detected")
        raise ValidationError("This notification already exists")
...
def get_notification(db: Database, notification_id: str, user_id: str) -> Notification:
    ...
    try:
        if not ObjectId.is_valid(notification_id):
            raise ValidationError(f"Invalid notification ID format: {notification_id}")
        ...
        if notification["user_id"] != user_id and notification["vendor_id"] != user_id:
...
def update_notification(db: Database, notification_id: str, user_id: str, update_data: dict) -> Notification:
    ...
    Args:
        ...
        update_data (dict): Data to update in the notification.
    ...
    try:
        ...
        updated = db.notifications.update_one({"_id": ObjectId(notification_id)}, {"$set": update_data})
...
def delete_notification(db: Database, notification_id: str, user_id: str) -> dict:
