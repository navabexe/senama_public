...
async def create_vendor_route(
    ...
    try:
        ...
        result = vendor_service.create_vendor(vendor_data.model_dump())
...
async def update_vendor_route(
    ...
    try:
        current_user = get_current_user(token, vendor_service.db)
        requester_id = str(current_user["_id"])
        vendor = vendor_service.update_vendor(vendor_id, requester_id, update_data.model_dump(exclude_unset=True))
...
async def delete_vendor_route(
    ...
    try:
        current_user = get_current_user(token, vendor_service.db)
        requester_id = str(current_user["_id"])
        result = vendor_service.delete_vendor(vendor_id, requester_id)
